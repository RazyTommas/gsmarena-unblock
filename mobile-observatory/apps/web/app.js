...
function renderRadar() {
  ...
  return heading('Update Radar','What changed since your last visit','A precise feed of new firmware for the devices and regions you care about.') + `
    ...
    <div class="validation-note">A dash (—) means the captured vendor artifact did not state that value — it is not an empty value and not a guess. Build names are never guessed into Android versions. Click any dash for a research brief, or a device name for its known history.</div><div class="subtle">${page.total?page.offset+1:0}–${(page.offset||0)+visible.length} of ${page.total||0} matching events</div><div class="feed">${cards || `<div class="empty"><b>No ${state.radarTab} updates match.</b><br>Try another region or change type. This does not mean source coverage is complete.</div>`}</div><div class="toolbar"><button class="button" id="radarPrev" ${page.offset?'':'disabled'}>← Previous 50</button><button class="button" id="radarNext" ${page.nextCursor?'':'disabled'}>Next 50 →</button></div>`;
}
...
async function loadSecurityPage(offset=0){const request=++securityPageRequest;const payload=await api.security({...state.securityFilters,limit:100,offset});if(request!==securityPageRequest)return;state.data.security=items(payload);state.data.securityPage=payload.meta?.page||{};render();}
function renderSecurity() {
  ...
  <div class="toolbar"><button class="button" id="securityPrev" ${page.offset?'':'disabled'}>← Previous 100</button><span class="subtle">Rows ${total?(page.offset||0)+1:0}–${(page.offset||0)+rows.length} of ${total}</span><button class="button" id="securityNext" ${page.nextCursor?'':'disabled'}>Next 100 →</button></div>
